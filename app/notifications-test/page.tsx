...
export default function NotificationsTestPage() {
  ...
  return (
    <div className="max-w-xl mx-auto p-6 space-y-6">
      <h1 className="text-2xl font-semibold">SNS Notification Test</h1>
      <p className="text-sm text-gray-600">Dev-only page. Sends a direct SMS (and optional topic publish) using the /api/notifications/test endpoint. Ensure AWS credentials & spending limits are configured. Do not deploy to production.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white/50 dark:bg-neutral-900/50 border border-gray-200 dark:border-neutral-700 rounded-lg p-4">
        <div>
          <label className="block text-sm font-medium mb-1">User Phone (E.164)</label>
          <input name="userPhone" value={form.userPhone} onChange={handleChange} placeholder="+15551234567" className="w-full border rounded px-3 py-2 text-sm" required />
        </div>
        ...
        <button type="submit" disabled={result.loading} className="px-4 py-2 rounded bg-indigo-600 text-white text-sm font-medium disabled:opacity-50">
          {result.loading ? 'Sending...' : 'Send Test Notification'}
        </button>
      </form>
