...
export default function SignupPage() {
  ...
  // Redirect to account type selection if no type is specified
  useEffect(() => {
  ...
  // Update user type when it changes in URL
  useEffect(() => {
  ...
  const handleUserTypeChange = (_value: string) => {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // If token returned, hydrate auth context
        ...
        setTimeout(() => {
          if (data.data.token) {
            // If token is provided, redirect to appropriate onboarding flow
            switch (data.data.user.userType) {
              case 'therapist':
                router.push('/therabook/onboarding/therapist');
                break;
              case 'instructor':
                router.push('/therabook/onboarding/instructor');
                break;
              case 'student':
                router.push('/therabook/onboarding/student');
                break;
              case 'clinic':
                router.push('/therabook/onboarding/clinic');
                break;
              case 'patient':
              default:
  ...
  const getUserTypeLabel = (_type: string) => 'Patient/User';
        ...
        <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
          ...
          <CardContent>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Account Type removed (only patient user allowed here) */}
