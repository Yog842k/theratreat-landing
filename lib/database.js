    ...
    const getCollection = (name) => {
      ...
      return {
        ...
        find: (filter = {}) => {
          ...
          // Add sort and limit chaining for compatibility
          return {
            sort(sortObj) {
          ...
          if (update && update.$pull) {
            for (const key in update.$pull) {
              if (Array.isArray(found[key])) {
                const cond = update.$pull[key];
                found[key] = found[key].filter((item) => {
                  // Only support object equality for {_id: ...} pattern
                  if (typeof cond === 'object' && cond !== null && '_id' in cond) {
                    return item._id !== cond._id;
                  }
                  // fallback: remove if exactly equal
                  return item !== cond;
                });
                modified = true;
              }
