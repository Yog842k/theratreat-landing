...
class DatabaseInitializer {
  ...
  await database.createIndex('bookings', { appointmentDate: 1, reminder24hSent: 1, status: 1 });
      ...
      // TheraStore collections indexes
      await database.createIndex('products', { isActive: 1 });
      await database.createIndex('products', { category: 1 });
      await database.createIndex('products', { subcategory: 1 });
      await database.createIndex('products', { brand: 1 });
      await database.createIndex('products', { price: 1 });
      await database.createIndex('products', { createdAt: -1 });
      await database.createIndex('products', { isFeatured: 1 });
      await database.createIndex('products', { tags: 1 });
      await database.createIndex('products', { sku: 1 }, { unique: false });

      await database.createIndex('therastore_vendors', { email: 1 }, { unique: true });
      await database.createIndex('therastore_vendors', { gstin: 1 }, { unique: true });
      await database.createIndex('therastore_vendors', { status: 1 });
      await database.createIndex('therastore_vendors', { createdAt: -1 });

      await database.createIndex('therastore_carts', { userId: 1 }, { unique: true });
      await database.createIndex('therastore_carts', { updatedAt: -1 });

      await database.createIndex('therastore_orders', { userId: 1 });
      await database.createIndex('therastore_orders', { status: 1 });
      await database.createIndex('therastore_orders', { createdAt: -1 });
