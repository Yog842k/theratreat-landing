...
const products: Product[] = [
  {
    id: "1",
    name: "Elite Pro Massage Therapy Table",
    brand: "TherapyPro",
    price: 15999,
    originalPrice: 18999,
    rating: 4.9,
    ...
    subcategory: "Therapy Tables",
    condition: "New",
    inStock: true,
    stockCount: 15,
    fastDelivery: true,
    description: "Professional-grade massage table with adjustable height and premium memory foam padding.",
    features: ["Adjustable Height", "Memory Foam", "Portable"],
    seller: "TherapyEquip Pro",
    isBestSeller: true,
    isDeal: true,
    conditionUse: ["Chronic Pain"],
    therapyType: ["Physiotherapy"],
    ageGroup: ["Adults"],
    deviceFunction: ["Treatment"],
    useLocation: ["Clinical Use"],
    availability: "In Stock"
  },
  {
    id: "2",
    name: "PhysioFlex Resistance Kit",
    brand: "FlexiFit",
    price: 899,
    originalPrice: 1299,
    rating: 4.8,
    ...
    stockCount: 250,
    fastDelivery: true,
    description: "Medical-grade resistance bands for progressive rehabilitation.",
    features: ["5 Levels", "Latex-Free", "Door Anchor"],
    seller: "RehabSupply",
    isBestSeller: true,
    isTopRated: true,
    conditionUse: ["Stroke Recovery"],
    therapyType: ["Physiotherapy"],
    ageGroup: ["All Ages"],
    deviceFunction: ["Training"],
    useLocation: ["Home Use"],
    availability: "In Stock"
  },
  {
    id: "3",
    name: "Sensory Integration Pod",
    brand: "SensoryPro",
    price: 2599,
    originalPrice: 3499,
    rating: 4.7,
    ...
    stockCount: 45,
    fastDelivery: true,
    description: "Complete sensory mat set for sensory processing disorder therapy.",
    features: ["Textured Surface", "Non-Slip", "Washable"],
    seller: "OT Solutions",
    isBestSeller: false,
    conditionUse: ["Autism Spectrum Disorder (ASD)"],
  ...
  {
    id: "4",
    name: "NeuroTalk Communication Board",
    brand: "TalkEasy",
    price: 1899,
    originalPrice: 2599,
    rating: 4.6,
    reviewCount: 234,
    image: "https://images.unsplash.com/photo-1607827448387-a67db1383b59?auto=format&fit=crop&q=80&w=800",
    ...
    stockCount: 78,
    fastDelivery: true,
    description: "Interactive communication board with picture exchange system.",
    features: ["PECS Ready", "Durable", "Portable"],
    seller: "SpeechPath Supplies",
    isTopRated: true,
    conditionUse: ["Speech Delay"],
    therapyType: ["Speech Therapy"],
    ageGroup: ["Children"],
    deviceFunction: ["Communication"],
    useLocation: ["Home Use"],
    availability: "In Stock"
  },
  {
    ...
    stockCount: 67,
    fastDelivery: true,
    description: "Professional TENS/EMS device for pain management.",
    features: ["16 Modes", "Rechargeable", "Dual Channel"],
    seller: "MedTech Solutions",
    isTopRated: true,
    conditionUse: ["Chronic Pain"],
    therapyType: ["Physiotherapy"],
    ageGroup: ["Adults"],
    deviceFunction: ["Pain Relief"],
    useLocation: ["Home Use"],
    availability: "In Stock"
  },
];
...
export default function TheraStore() {
  ...
  // Cart Actions
  const addToCart = (productId: string, quantity: number = 1) => setCart(prev => ({ ...prev, [productId]: (prev[productId] || 0) + quantity }));
  const removeFromCart = (productId: string) => setCart(prev => { const newCart = { ...prev }; delete newCart[productId]; return newCart; });
  const updateQuantity = (productId: string, quantity: number) => quantity <= 0 ? removeFromCart(productId) : setCart(prev => ({ ...prev, [productId]: quantity }));
  ...
  if (currentView === "cart") {
    return <ShoppingCartView cartItems={cartItems} onBack={() => setCurrentView("products")} onUpdateQuantity={updateQuantity} onRemove={removeFromCart} total={cartTotal} />;
  }

  return (
    <div className="w-full min-h-screen bg-slate-50/50 font-sans text-slate-900 selection:bg-emerald-100 selection:text-emerald-900">
          ...
          <div className="flex h-20 items-center justify-between gap-8">
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-xl font-bold tracking-tight text-slate-900 leading-none">TheraStore</span>
                <span className="text-[10px] font-semibold text-emerald-600 tracking-wider uppercase mt-0.5">by TheraTreat</span>
              </div>
            </motion.div>
            ...
            <div className="hidden lg:flex flex-1 max-w-lg relative group">
              ...
              <Input 
                placeholder="Search for therapy tools, brands, or needs..." 
                className="pl-11 h-12 bg-slate-100/50 border-transparent focus:border-emerald-200 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 rounded-full transition-all duration-300 font-medium text-slate-600 placeholder:text-slate-400"
                value={searchQuery}
            ...
            <div className="flex items-center gap-3">
               <Button variant="ghost" size="icon" className="lg:hidden"><Search className="h-5 w-5" /></Button>
               <Button variant="ghost" size="sm" className="hidden md:flex gap-2 font-semibold text-slate-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-full px-5 h-10"><User size={18}/> Login</Button>
               <div className="h-6 w-px bg-slate-200 hidden md:block mx-1"></div>
               <Button variant="ghost" size="icon" className="hover:bg-rose-50 hover:text-rose-500 transition-colors rounded-full h-10 w-10"><Heart size={20} /></Button>
               <Button 
                  variant="ghost" 
                  size="icon" 
                  className="relative hover:bg-emerald-50 hover:text-emerald-600 transition-colors rounded-full h-10 w-10" 
               ...
               >
                   ...
                   {cartCount > 0 && (
                     <motion.span 
                        initial={{ scale: 0 }} 
                        animate={{ scale: 1 }}
                        className="absolute top-1 right-1 h-4 w-4 bg-emerald-600 text-white text-[10px] flex items-center justify-center rounded-full font-bold ring-2 ring-white"
                     >
                       {cartCount}
                     </motion.span>
                   )}
        ...
        <main>
          ...
          <section className="py-24 bg-slate-900 text-white relative overflow-hidden">
             ...
             <div className="max-w-[1400px] mx-auto px-6 relative z-10">
                 ...
                 >
                     {[
                         { title: "Recover from Injury", desc: "Orthopedic supports & aids", icon: "🩹", color: "from-blue-500/20 to-blue-600/20" },
                         { title: "Neuro Rehabilitation", desc: "Stroke & spinal recovery tools", icon: "🧠", color: "from-purple-500/20 to-purple-600/20" },
                         { title: "Child Development", desc: "Sensory & motor skills", icon: "🧸", color: "from-pink-500/20 to-pink-600/20" },
                         { title: "Pain Management", desc: "Relief for chronic conditions", icon: "⚡", color: "from-amber-500/20 to-amber-600/20" },
                         { title: "Improve Mobility", desc: "Walkers, canes & wheelchairs", icon: "🚶", color: "from-emerald-500/20 to-emerald-600/20" },
                         { title: "Daily Living Aids", desc: "Tools for independence", icon: "🏠", color: "from-teal-500/20 to-teal-600/20" },
                     ].map((goal, i) => (
                         <motion.div 
                            key={i}
                            variants={itemVar}
                            whileHover={{ scale: 1.03 }}
                            className={`p-8 rounded-3xl bg-gradient-to-br ${goal.color} border border-white/5 hover:border-white/20 transition-all cursor-pointer backdrop-blur-sm group`}
                         ...
                         >
                             ...
                             <p className="text-slate-400 group-hover:text-slate-200 transition-colors">{goal.desc}</p>
                         </motion.div>
                     ))}
                 </motion.div>
             </div>
...
function ShoppingCartView({ cartItems, onBack, onUpdateQuantity, onRemove, total }: any) {
    return (
                ...
                {cartItems.length === 0 ? (
                    ...
                    <div className="grid lg:grid-cols-3 gap-10">
                        ...
                        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 h-fit sticky top-24">
                            ...
                            <Button className="w-full h-14 rounded-2xl bg-slate-900 hover:bg-emerald-600 text-white font-bold text-lg shadow-lg shadow-emerald-900/10 transition-all hover:scale-[1.02]">Proceed to Checkout</Button>
